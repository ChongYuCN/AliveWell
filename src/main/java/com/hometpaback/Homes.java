...
import com.chongyu.aliveandwell.hometpaback.util.TextUtils;
...
public class Homes {
    ...
    int homeInit(CommandContext<ServerCommandSource> ctx, String name) throws CommandSyntaxException {
        ...
        if (home.isEmpty()) {
            ctx.getSource().sendFeedback(() -> {
                return Text.translatable("aliveandwell.hometpaback.nohome").formatted(Formatting.RED);
            }, false);
            return 0;
        }
        ...
        TeleportUtils.genericTeleport(true, 3, player, () -> {
            ...
            if((player.totalExperience >= experience && player.getWorld().getRegistryKey().equals(home.get().getDimID())  && (st1.equals(st2) && st1.contains("minecraft")))
            ...
            } else if(!player.getWorld().getRegistryKey().equals(home.get().getDimID()) && (st1.equals(st2) && !st1.contains("minecells")) && !st1.contains("twilightforest")
                    || !st1.equals(st2)
            ){
                ctx.getSource().sendFeedback(() -> {
                    return Text.translatable("aliveandwell.hometpaback.nosameworld").formatted(Formatting.RED);
    ...
    int homeSet(CommandContext<ServerCommandSource> ctx, String name) throws CommandSyntaxException {
        ...
        if (((IStoreHome)player).getHomes().size() >= maxHomes) {
            ctx.getSource().sendFeedback(() -> {
                return Text.translatable("aliveandwell.hometpaback.maxhomes").formatted(Formatting.RED);
            }, false);
            return 1;
        }
        ...
        if (((IStoreHome)player).addHome(new HomeComponent(
            ...
            if (home.isEmpty()) {
                ctx.getSource().sendFeedback(() -> {
                    return Text.translatable("aliveandwell.hometpaback.homewrong").formatted(Formatting.RED);
                }, true);
                return 1;
            }
            ...
            ctx.getSource().sendFeedback(() -> {
                return Text.translatable("aliveandwell.hometpaback.sethome",
            ...
            ctx.getSource().sendFeedback(() -> {
                return Text.translatable("aliveandwell.hometpaback.hashome").formatted(Formatting.RED);
            }, false);
            return 1;
        }
        return 1;
    ...
    int homeDel(CommandContext<ServerCommandSource> ctx, String name) throws CommandSyntaxException {
        ...
        if (((IStoreHome)player).removeHome(name)) {
            ...
            if (home.isPresent()) {
                ctx.getSource().sendFeedback(() -> {
                    return Text.translatable("aliveandwell.hometpaback.removehomewrong").formatted(Formatting.RED);
                }, true);
                return 1;
            }

            ctx.getSource().sendFeedback(() -> {
                return Text.translatable("aliveandwell.hometpaback.removehome",
                        Text.literal(name).formatted(Formatting.GOLD)).formatted(Formatting.LIGHT_PURPLE);
            }, false);
        } else {
            ctx.getSource().sendFeedback(() -> {
                return Text.translatable("aliveandwell.hometpaback.noremovehome").formatted(Formatting.RED);
            }, false);
            return 1;
        }
        return 1;
    ...
    int homeList(CommandContext<ServerCommandSource> ctx, ServerPlayerEntity player) {
        ...
        ctx.getSource().sendFeedback(() -> {
            return Text.translatable("%s/%s:\n", homes.size(), maxHomes).append(TextUtils.join(list, Text.literal(", ")));
        }, false);
        return 1;
    }
}
