...
import com.chongyu.aliveandwell.util.CanJoinEnd;
        ...
        if(entity instanceof ServerPlayerEntity player){
            if(AliveAndWellMain.canEnd){//test
                ...
                if(FabricLoader.getInstance().isModLoaded("illagerinvasion")) {
                    try {
                        if (!CanJoinEnd.canJoinEnd19(player)) {
                            player.sendMessage(Text.translatable("aliveandwell.endportal.advancement19").formatted(Formatting.YELLOW));
                        }
                        if (!CanJoinEnd.canJoinEnd20(player)) {
                            player.sendMessage(Text.translatable("aliveandwell.endportal.advancement20").formatted(Formatting.YELLOW));
                        }
