...
//                   }//test=================================================
                    if(target instanceof SilverfishEntity){
                        if(player.getWorld().getRegistryKey().getValue().toString().contains("twilightforest")){
...
//                            player.sendMessage(Text.translatable("生物群落名字一："+world.getBiome(entity.getBlockPos()).getKey().get().getValue().toString()));
                   ...
                   if (mob.contains("terrarianslimes.")) {
                       int random1 = player.getWorld().random.nextInt(11);
                       int random2 = player.getWorld().random.nextInt(5);
                       ...
                       if (mob.contains("black_slime")) { //黑史莱姆:black_slime腐蚀物品装备
                           if(random1 == 0 ){
                               ...
                               //快捷栏和物品栏
                               damageItem(player,player.getInventory().main.get(player.getWorld().random.nextInt(36)));//一般物品
                           }else if(random1 == 10){
                               damageFood(player,player.getInventory().main.get(player.getWorld().random.nextInt(36)));//食物
                           }else if(random1 == 6){
...
//                   player.sendMessage(Text.translatable("getAttacking的名字："+ player.getAttacking().getName().toString()));//攻击玩家的怪物
//                   player.sendMessage(Text.translatable("getAttacker的名字："+ player.getAttacker().getName().toString()));//攻击玩家的怪物
//                   //玩家是否被攻击
//                   player.sendMessage(Text.translatable("player.tryAttack(player.getAttacker())的名字："+ player.tryAttack(player.getAttacker())));
//                   player.sendMessage(Text.translatable("player.tryAttack(player.isAttackable())的名字："+ player.isAttackable()));
               }
           }
            return damageAmount;
        });
    ...
    public static void durability(PlayerEntity player, ItemStack stack){
        if(stack.getItem() instanceof ToolItem || stack.getItem() instanceof ArmorItem){
            stack.damage(2, player.getWorld().random,(ServerPlayerEntity) player);
        }
    }
    //腐蚀物品数量
    public static void damageItem(PlayerEntity player,ItemStack stack){
        if(!stack.getItem().isFood() && !(stack.getItem() instanceof ToolItem) || !(stack.getItem() instanceof ArmorItem)){
            stack.increment(-(player.getWorld().random.nextInt(2)));
    ...
    public static void damageFood(PlayerEntity player,ItemStack stack){
        if(stack.getItem().isFood()){//腐蚀食物
            stack.increment(-(player.getWorld().random.nextInt(3)));
            player.getWorld().playSound(player.getX(),player.getY(),player.getZ(), SoundEvents.BLOCK_LAVA_EXTINGUISH, SoundCategory.PLAYERS,1.0f,1.0f,true);
        }
    }
    //行动缓慢效果
