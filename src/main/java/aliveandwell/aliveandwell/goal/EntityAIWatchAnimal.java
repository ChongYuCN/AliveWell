...
//                        Vec3Pool var10 = world.getWorldVec3Pool();
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (isAirOrPassableBlock(target.getBlockPos().getX(), target.getBlockPos().getY() + target.getHeight() * 0.75 + 1, target.getBlockPos().getZ(), false, this.zombie.world))
                                {
                                    ...
                                    rc = DigHelper.getBlockCollisionForPhysicalReach(this.digger.getEyePosForBlockDestroying(), target_center_pos.add(new Vec3d(0.0F, 1.0F, 0.0F)), zombie.getEntityWorld(), zombie);
                                ...
                                rc = DigHelper.getBlockCollisionForPhysicalReach(this.digger.getEyePosForBlockDestroying(), target_center_pos, this.zombie.getEntityWorld(), zombie);
                                ...
                                rc = DigHelper.getBlockCollisionForPhysicalReach(this.digger.getAttackerLegPosForBlockDestroying(), target_center_pos, this.zombie.getEntityWorld(), zombie);
    ...
    {
        return DigHelper.getBlockCollisionForPhysicalReach(origin, limit, world, zombie);
    }
    ...
    private boolean couldHitTargetByPathing()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                return !(DigHelper.getDistanceFromDeltas((double) x - target.getX(), (double) y - target.getY(), (double) z - target.getZ()) > 1.0F) && !this.getIntersectingBlock(new Vec3d(x, y, z), this.digger.getTargetEntityCenterPosForBlockDestroying(target), this.zombie.getEntityWorld()).isBlock();
            }
        }
    }
    ...
    private boolean couldGetCloserByPathing()
    {
        ...
        {
            double distance = DigHelper.getDistanceFromDeltas(this.zombie.getX() - target.getX(), this.zombie.getY() - target.getY(), this.zombie.getZ() - target.getZ());
            Path path = this.zombie.getNavigation().findPathTo(target, 16);
            ...
            else
            {
                ...
                float y = (float)final_point.y;
                float z = (float)final_point.z + 0.5F;
                return (double) DigHelper.getDistanceFromDeltas((double)x - target.getX(), (double)y - target.getY(), (double)z - target.getZ()) < distance - 2.0D;
            }
        }
    }
