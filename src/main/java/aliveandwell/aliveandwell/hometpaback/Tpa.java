...
public class Tpa {
    ...
    public int tpaInit(CommandContext<ServerCommandSource> ctx, ServerPlayerEntity tTo) throws CommandSyntaxException {
        ...
        String st1 = tr.tFrom.world.getRegistryKey().toString().substring(0,tr.tFrom.world.getRegistryKey().toString().indexOf(":"));
        String st2 = tr.tTo.world.getRegistryKey().toString().substring(0,tr.tTo.world.getRegistryKey().toString().indexOf(":"));
        if((!tr.tFrom.world.getRegistryKey().equals( tr.tTo.world.getRegistryKey())) && (((st1.equals(st2) && !st1.contains("minecells"))) && ((st1.equals(st2) && !st1.contains("twilightforest"))))
                || !st1.equals(st2)
        ){
    ...
    public int tpaAccept(CommandContext<ServerCommandSource> ctx, ServerPlayerEntity rFrom) throws CommandSyntaxException {
        ...
        int experience = ((int)(tr.rFrom.world.getLevelProperties().getTimeOfDay()/ 24000L)+1)*xpCostTime;
        if (experience >= 1000 ){
            experience = 1000;
        }
        ...
        TeleportUtils.genericTeleport(true, stand_still, rFrom, () -> {
            ...
            int experience1 = ((int)(tr.tFrom.world.getLevelProperties().getTimeOfDay()/ 24000L)+1)*xpCostTime;
            if (experience1 >= 1000 ){
                experience1 = 1000;
            }

            //经验足够且同一世界
            if((tr.rFrom.totalExperience >= experience1 && tr.tFrom.world.getRegistryKey() == tr.tTo.getWorld().getRegistryKey()
                    && (st1.equals(st2) && st1.contains("minecraft")))
                    || (st1.equals(st2) && st1.contains("minecells")) || (st1.equals(st2) && st1.contains("twilightforest"))
            ){
                //设置back点
                ((IStoreHome)tr.tFrom).addBack(new Vec3d(tr.tFrom.getX(), tr.tFrom.getY(), tr.tFrom.getZ()),tr.tFrom.world.getRegistryKey());
            ...
            }else if(tr.rFrom.totalExperience < experience1 && tr.rFrom.world.getRegistryKey().equals(tr.tTo.world.getRegistryKey())){
            ...
            }else if((!tr.rFrom.world.getRegistryKey().equals(tr.tTo.world.getRegistryKey()) && (st1.equals(st2) && st1.contains("minecraft")))
                    || !st1.equals(st2)
            ){
