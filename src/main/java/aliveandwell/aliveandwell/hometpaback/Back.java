...
public class Back {
    ...
    int backInit(CommandContext<ServerCommandSource> ctx, Object o) throws CommandSyntaxException {
        ...
        if(back.getLeft() != null && back.getRight() != null) {
            ...
            int experience1 = ((int)(player.world.getLevelProperties().getTimeOfDay()/ 24000L)+1)*xpCostTime;
            if (experience1 >= 1000 ){
                experience1 = 1000;
            }
            ...
            String st1 = player.world.getRegistryKey().toString().substring(0,player.world.getRegistryKey().toString().indexOf(":"));
            String st2 = back.getRight().toString().substring(0,back.getRight().toString().indexOf(":"));
            if(!player.world.getRegistryKey().equals(back.getRight()) && (st1.equals(st2) && !st1.contains("minecells")) && !st1.contains("twilightforest")
                    || !st1.equals(st2)
            ){
            ...
            TeleportUtils.genericTeleport(true, 3, player, () -> {
                int experience = ((int)(player.world.getLevelProperties().getTimeOfDay()/ 24000L)+1)*xpCostTime;
                if (experience >= 1000 ){
                    experience = 1000;
                }

                if((player.totalExperience >= experience && player.world.getRegistryKey().equals(back.getRight())  && (st1.equals(st2) && st1.contains("minecraft")))
                    ...
                    ((SPlayerAccessorNbt) (Object)player).splayerNbt$invulnerableFlag(true);//设置无敌
                ...
                }else if (player.totalExperience < experience && player.world.getRegistryKey().equals(back.getRight())){//
                    ctx.getSource().sendFeedback(Text.translatable("aliveandwell.hometpaback.xpno1").append(Text.literal(String.valueOf(experience))).append(Text.translatable("aliveandwell.hometpaback.notp")).formatted(Formatting.RED), false);
                } else if(!player.world.getRegistryKey().equals(back.getRight()) && (st1.equals(st2) && !st1.contains("minecells")) && !st1.contains("twilightforest")
                        || !st1.equals(st2)
                ){
                    ctx.getSource().sendFeedback(Text.translatable("aliveandwell.hometpaback.nosameworld").formatted(Formatting.RED), false);
                }
    ...
    int backSet(CommandContext<ServerCommandSource> ctx, Object o) throws CommandSyntaxException {
        ServerPlayerEntity player = ctx.getSource().getPlayerOrThrow();
        ((IStoreHome) player).addBack(new Vec3d(player.getX(), player.getY(), player.getZ()), player.world.getRegistryKey());
