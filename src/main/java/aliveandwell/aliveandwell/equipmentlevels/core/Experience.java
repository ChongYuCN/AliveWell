...
public class Experience {
    public static int getNextLevel(PlayerEntity player, ItemStack stack, NbtCompound nbt, int currentLevel, int experience) {
        ...
        while (currentLevel < Static.MAX_LEVEL && experience >= Experience.getMaxLevelExp(currentLevel)) {
            ...
            int canLevel = 1 ;
            while (canLevel == 1){
                Ability ability_weapon = Ability.WEAPON_ABILITIES.get(player.world.random.nextInt(Ability.WEAPON_ABILITIES.size()));
                Ability  ability_armor = Ability.ARMOR_ABILITIES.get(player.world.random.nextInt(Ability.ARMOR_ABILITIES.size()));
                ...
                if(EAUtil.canEnhance(stack.getItem()) && EAUtil.canEnhanceWeapon(stack.getItem())){
                    if(!ability_weapon.hasAbility(nbt)){
                        ...
                        player.sendMessage(Text.translatable("aliveandwell.levelupinfo").formatted(Formatting.LIGHT_PURPLE).append(Text.of(Formatting.GOLD +"【")).append(stack.getName().copy().formatted(Formatting.GOLD)).append(Text.of( Formatting.GOLD +"】")) .append(Text.translatable("enhancedarmaments.misc.ability.get").formatted(Formatting.LIGHT_PURPLE)).copy().append(Text.translatable(ability_weapon.getName(nbt)).copy().formatted(Formatting.GOLD)).append(Text.translatable(Formatting.GRAY  + " !")) );
                        canLevel = 0;
                    }else {
                ...
                if(EAUtil.canEnhance(stack.getItem()) && EAUtil.canEnhanceArmor(stack.getItem())){
                    if(!ability_armor.hasAbility(nbt)){
                        ...
                        player.sendMessage(Text.translatable("aliveandwell.levelupinfo").formatted(Formatting.LIGHT_PURPLE).append(Text.of(Formatting.GOLD +"【")).append(stack.getName().copy().formatted(Formatting.GOLD)).append(Text.of( Formatting.GOLD +"】")) .append(Text.translatable("enhancedarmaments.misc.ability.get").formatted(Formatting.LIGHT_PURPLE)).copy().append(Text.translatable(ability_armor.getName(nbt)).copy().formatted(Formatting.GOLD)).append(Text.translatable(Formatting.GRAY  + " !")) );
                        canLevel = 0;
                    }else {
